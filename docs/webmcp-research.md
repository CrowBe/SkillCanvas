...
The current draft describes in-page execution as `Promise<DOMString> executeTool(RegisteredTool tool, optional object inputObject = {})` and says the browser JSON-serializes the `execute` return value. In practice Chrome 151 (`--enable-features=WebMCP --enable-blink-features=WebMCP`, `Google Chrome 151.0.7922.71`) **requires the second argument to be a JSON string** (`DOMString`): passing a plain object consistently rejects as `UnknownError: Failed to parse input arguments`, while `JSON.stringify(input)` succeeds for every workbench tool (verified 2026-09-03 against `http://127.0.0.1:5173/` with 12 registered tools). The browser parses that string before invoking `execute(input, { signal })`, so the workbench handler receives an object. The handler is intentionally tolerant — a string input is `JSON.parse`d — so both wires work if the caller side varies by build. Source for the wire divergence: manual Chrome 151 probe; draft IDL still says `object` at the research date. Source for the intended serialization: [imperative execution serialization algorithm](https://webmachinelearning.github.io/webmcp/#imperative-execute-steps).
